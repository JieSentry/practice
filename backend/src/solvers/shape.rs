...
pub struct TransparentShapeSolver {  
    ...
    merge_frames: u32,                       // 融合/重叠模式持续帧数  
    last_track_rect: Option<Rect>,           // 上一帧目标的 bounding box  
    pre_overlap_velocity: Option<Point2d>,   // 进入重叠前保存的速度  
    ...
    pub fn solve(&mut self, detector: &dyn Detector, region: Rect) -> Option<Point> {  
        ...
        let Some(best) = best_track else {  
            ...
            let next_cursor = if let Some(disp) = displacement {  
                ...
                let nc = last_cursor  
                    + Point::new(vel.x.round() as i32, vel.y.round() as i32);  
                Point::new(  
                    nc.x.clamp(0, region.width - 1),  
                    nc.y.clamp(0, region.height - 1),  
                )  
            ...
            debug!(target: "backend/player",  
                "no track found, frame {}: cursor {:?} -> {:?}",  
                self.merge_frames, last_cursor, next_cursor);  
        ...
        let is_overlapping = tracks.iter().any(|t| {  
            !std::ptr::eq(t, best) && (best_rect & t.rect()).area() > 0  
        });  
        ...
        if is_overlapping {  
            ...
            if self.merge_frames > FPS / 2 {  
                debug!(target: "backend/player",  
                    "overlap timeout after {} frames, resetting", self.merge_frames);  
                self.last_cursor = None;  
                self.last_velocity = None;  
                self.last_track_rect = None;  
                self.pre_overlap_velocity = None;  
            ...
            debug!(target: "backend/player",  
                "overlap mode frame {}: cursor {:?} -> {:?}, displacement {:?}",  
