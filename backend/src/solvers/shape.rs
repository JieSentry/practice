...
/// 重认领参照点沿运动方向前移的固定像素数（恒定 30px，不随速度缩放）。  
const FORWARD_SHIFT_PX: f64 = 30.0;  
...
impl TransparentShapeSolver {  
    ...
    fn update_and_find_best_track<'a>(  
        ...
        // 在所有逆背景候选里选离「前移参照点」最近的那个，采纳其新 ID。  
...
/// 前移参照点 = 上一帧光标 + 沿运动方向恒定前移 FORWARD_SHIFT_PX 像素。  
///  
/// 注意：这里 **不再** 用 `last_cursor + last_velocity`（前移量随速度缩放、  
/// 且方向误差随速度放大），改为把速度归一化成单位方向后乘固定 30px，  
/// 使前移量恒定、可预测；速度过小（unit 返回 None）时退回裸 last_cursor。  
fn predicted_cursor(last_cursor: Point, last_velocity: Option<Point2d>) -> Point {  
    match last_velocity.and_then(unit) {  
        Some(dir) => {  
            last_cursor  
                + Point::new(  
                    (dir.x * FORWARD_SHIFT_PX).round() as i32,  
                    (dir.y * FORWARD_SHIFT_PX).round() as i32,  
                )  
        }  
        None => last_cursor,  
    }  
