...
fn detect_tof_next_button(&self) -> Result<Rect> {    
    ...
    fn detect_lie_detector_shape_preparing(&self) -> bool {  
        detect_lie_detector_shape_preparing(self.bgr(), &self.localization).is_ok()  
    }
...
pub static LIE_DETECTOR_SHAPE_PREPARE_TEMPLATE: LazyLock<Mat> = LazyLock::new(|| {  
    imgcodecs::imdecode(  
...
fn detect_lie_detector_shape_preparing(bgr: &impl ToInputArray, localization: &Localization) -> Result<Rect> {  
    let template = localization  
        .lie_detector_shape_prepare_base64  
        .as_ref()  
        .and_then(|base64| to_mat_from_base64(base64, false).ok());  
  
    detect_template(  
        bgr,  
        template.as_ref().unwrap_or(&*LIE_DETECTOR_SHAPE_PREPARE_TEMPLATE),  
        Point::default(),  
        0.6,  
    )  
}
