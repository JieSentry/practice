...
pub struct ThreadsOfFateState {
    ...
    /// Number of consecutive ask failures (used to detect if 7 quests are done)
    ask_fail_count: u32,
...
fn update_find_complete(resources: &mut Resources, tof: &mut ThreadsOfFateState) {
    ...
    // Check if we've reached target complete count
    if tof.complete_executed_count >= tof.target_complete_count {
        info!(target: "backend/player", "threads of fate: target complete count reached ({}), checking if all quests done", tof.complete_executed_count);
        if tof.ask_fail_count >= 1 {
            info!(target: "backend/player", "threads of fate: ask failed previously, all quests completed, stopping");
...
fn update_interact_complete(resources: &mut Resources, tof: &mut ThreadsOfFateState) {
    ...
    match next_timeout_lifecycle(timeout, INTERACT_PRESS_INTERVAL) {
        ...
        tof.state = State::InteractComplete(Timeout::default(), new_count, miss_count + 1);
                ...
                // After completing, continue to unravelling to accept next task
                // Only stop if we've reached the target AND ask has failed (indicating all quests done)
                if tof.complete_executed_count >= tof.target_complete_count && tof.ask_fail_count >= 1 {
                    info!(target: "backend/player", "threads of fate: all {} complete quests executed and ask failed, permanently stopping", tof.target_complete_count);
                    tof.success = true;
                    tof.permanently_stopped = true;
                    tof.completed = true;
                } else {
                    info!(target: "backend/player", "threads of fate: complete finished, re-opening mailbox to accept next task");
...
fn update_click_ask(resources: &mut Resources, tof: &mut ThreadsOfFateState) {
    ...
    match next_timeout_lifecycle(timeout, 30) {
        ...
        Lifecycle::Ended => {
            ...
            if resources.detector().detect_tof_dialog_visible() {
                // Dialog appeared, reset ask_fail_count and proceed
            ...
            if new_retry >= MAX_ASK_FAIL_COUNT {
                ...
                // Increment global ask_fail_count
                tof.ask_fail_count += 1;
                
                // If ask failed twice (two cycles), permanently stop - all 7 quests are done
                if tof.ask_fail_count >= 2 {
                    info!(target: "backend/player", "threads of fate: ask failed in two consecutive cycles ({} total fails), all quests completed, permanently stopping", tof.ask_fail_count);
                    tof.permanently_stopped = true;
                    tof.completed = true;
                } else {
                    info!(target: "backend/player", "threads of fate: ask failed {} times total, returning to idle for retry", tof.ask_fail_count);
                    tof.completed = true;
                }
            } else {
